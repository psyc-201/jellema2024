...
Google Gemini was used to generate the face morph stimuli. The experiment was coded in JsPsych.

> Participants were presented with short video clips (2s) depicting the frontal face view of an actor (agent A or agent B; Figure 1). Their facial expressions and gaze directions changed smoothly over the course of the clips, displaying a natural facial movement. (Jellema et al., 2024, p. 5)
>
...
First, participants signed the consent form and then answered the AQ-10. The experiment began with instructions that showed participants identities A and B making neutral faces side by side, in addition to instructing them to focus on the videos and be ready for attention checks. Next, participants were shown the video clips (described in materials) forwards and backwards the same number of times and in a randomized order. There were 56 acquisition trials split across two blocks with a break in between. There were four attention checks spaced roughly evenly across the acquisition trials that asked participants to select the correct color word (i.e. If you are paying attention, select 'yellow.'). The test trials began with a screen displaying the instructions. Then, participants were shown each set of five test images two times, alternating between the smiling and frowning conditions. Next, participants answered two questions probing whether they had detected the contingencies in the experiment. Lastly, participants were debriefed and provided their completion information for Prolific. The experiment took approximately 10-15 minutes to complete.

## Analysis Plan

The first few steps of preparing the data for analysis is importing the data and parsing the JSONs that the questionnaire data is imported as. Additionally, strings need to be converted into numeric data for successful analysis. Next, the implicit learning measure is calculated, as well as total scores on the AQ-10. Then, participants are excluded. Participants are excluded for detecting the contingencies in the experiment, answering too fast or too slow on the test trials, missing responses on the test trials, failing attention checks, and taking too long to complete attention checks. Then, I move on to analysis.

The analysis begins with viewing the demographic data from the sample. Next, means of scores on the questionnaire and test trials are calculated. Then plots of accuracy and AQ-10 scores are created to get a visual sense of the distribution of the data. Following this, a figure comparing mean scores on each proportion across dispositions is created. Then, the key analysis, a two-way repeated measures ANOVA, is performed. Lastly, the correlation between autistic traits (AQ-10) and implicit learning is assessed and graphed. Below is the code used to prepare the data and perform the data analysis.
...
#import csv
data <- read.csv(file = "/Users/wilderhartwell/Documents/jellema2024/data/Data_collection_1/final_data_JSON.csv", header = FALSE)
...
#### Score correct responses and create figure

```{r score correct responses and create figure}
# score correct responses and create figure

# Load data
correct_response <- read.csv("/Users/wilderhartwell/Documents/jellema2024/data/Data_collection_1/correct_response.csv")

# Merge A_data with correct-response key
A_scored <- A_data %>%
  left_join(correct_response, by = "Proportion")

# Create accuracy column (1 = correct, -1 = incorrect)
A_scored <- A_scored %>%
  mutate(
    accuracy = if_else(Response == CorrectResponse, 1, -1)
  )
# Get mean score for each proportion across dispositions
accuracy_summary <- A_scored %>%
  group_by(Disposition.y, Proportion) %>%   
  summarise(
...
#export CSV to rename Proportion values
write.csv(accuracy_summary, file = "accuracy_summary.csv")

#import CSV back in to use in the graph
accuracy_summary <- read.csv(file = "/Users/wilderhartwell/Documents/jellema2024/writeup/accuracy_summary.csv")

# set correct factor structure to ensure x axis is correctly displayed
accuracy_summary$Proportion <- factor(
  accuracy_summary$Proportion,
  levels = c("60% A 40% B", 
             "55% A 45% B",
             "50% A 50% B",
             "45% A 55% B",
             "40% A 60% B"))
#create graph
jellemaplot <- ggplot(accuracy_summary, aes(x = Proportion, y = mean_accuracy, color = Disposition.y, group = Disposition.y)) +
  
  geom_line(size = 1.4) +
  
  scale_color_manual(
    values = c(
      "frown" = "#B22222",  # deep red
      "smile"  = "#1E90FF"   # blue
    ),
    name = NULL,
    labels = c("frowning morph", "smiling morph")
  ) +
...
There was no significant main effect of Disposition, meaning participants did not judge the smiling faces to be more like identity A (positive disposition), nor the frowning faces more like identity B (negative disposition). (F(1,27) = 1.223, p = 0.279, ηp^2^ = 0.04). There was also no main effect of Proportion (F(1,27) = 0.016, p = 0.9, ηp^2^ = 0.00). There was no significant interaction between disposition and proportion (F(1,27) = 0.759, p = 0.391, ηp^2^ = 0.03). See how participants performed for each disposition and proportion in the replication compared to Jellema et al. (2024) in Figure 1.

![](comparison plot.png)

Participants with higher autistic traits were expected to perform worse on the task, but this study found that AQ-10 scores and implicit learning were not significantly correlated (r(26) = -0.037, p = 0.851, 95% CI \[-0.405, 0.341\], two-tailed) (Figure 2).

![Figure 2. The correlation between AQ-10 scores and the measure of implicit learning.](corplot.png)

### Exploratory analyses

As depicted in Figure 3., the majority of participants performed at chance on the task. However, 10 participants scored below chance, while only six participants scored above chance. The mean task performance is 1.75 out of four points, which is below chance.

![Figure 3. Distribution of total scores on test trials.](accdistribution.png)

The mean score of 5.93 on the AQ-10 for this sample is very close to the threshold for possible autism of six out of 10 points. This shows a relatively high level of autistic traits in the sample. Looking at Figure 4., we can see that the most common score was seven, which is above the threshold. This means the majority of scores were at or above the threshold.

![Figure 4. Distribution of AQ-10 scores.](AQ-10distribution.png)

# Discussion

## Summary of Replication Attempt

My analyses found no main effect of disposition or proportion, contrary to Jellema et al. (2024) that found a main effect of both. Neither study found an interaction effect. Additionally, the replication found no correlation between AQ-10 scores and implicit learning. These results mean that Jellema et al. (2024) did not replicate.
